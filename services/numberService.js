...
async function buyAndSaveNumber({ client_id, country, assistant_id }) {
    ...
    const result = await db.query(
        `INSERT INTO phone_numbers (client_id, assistant_id, phone_number, provider, status)
         VALUES ($1, $2, $3, $4, 'active') RETURNING *`,
        [client_id, assistant_id || null, phoneNumber, providerName]
    );
...
async function linkNumberToAssistant({ phone_number_id, assistant_id }) {
    const result = await db.query(
        `UPDATE phone_numbers SET assistant_id = $1 WHERE id = $2 RETURNING *`,
        [assistant_id, phone_number_id]
    );
    if (result.rows.length === 0) throw new Error('Phone number not found');
    return result.rows[0];
}
