...
export default function WorkspaceSubscription() {
  ...
  // Razorpay Checkout State (Legacy)
  ...
  const triggerCheckout = async (planName: string) => {
    try {
      ...
      const result = await billingApi.subscribe(planName);
      if (result.url || result.checkoutUrl) {
        window.location.href = result.url || result.checkoutUrl;
      } else {
        throw new Error("Dodo Payments checkout link not generated by the server.");
      }
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-7xl mx-auto z-10 relative">
      ...
      {errorMsg && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-red-600 font-bold text-body-xs text-left animate-fade-in flex items-center gap-2">
            ...
            <div className={`border p-6 rounded-[22px] text-left relative flex flex-col justify-between ${
              ...
              {!isStarter ? (
                <button
                  ...
                  className="w-full bg-[#14141A] hover:bg-[#3A3A42] text-white font-bold text-[10px] uppercase tracking-wider py-3.5 rounded-xl transition-all disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-1.5"
                >
                  {subscribing === "starter" && <Spinner className="w-4 h-4 animate-spin" />}
                  <span>Activate Starter Tier</span>
                </button>
              ) : (
            ...
            <div className={`border p-6 rounded-[22px] text-left relative flex flex-col justify-between ${
              ...
              {!isGrowth ? (
                <button
                  ...
                  disabled={subscribing !== null}
                  className="w-full bg-[#FF6B00] hover:bg-[#FF8C3A] text-white font-bold text-[10px] uppercase tracking-wider py-3.5 rounded-xl transition-all shadow-[0_4px_12px_rgba(255,107,0,0.15)] disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-1.5"
                >
                  {subscribing === "growth" && <Spinner className="w-4 h-4 animate-spin" />}
                  <span>Activate Growth Tier</span>
                </button>
              ) : (
                <div className="w-full bg-[#E5E0D8] text-[#5A5A66] font-bold text-center text-[10px] uppercase tracking-wider py-3.5 rounded-xl border border-[#D8D2C4] select-none">
                  Currently Subscribed
