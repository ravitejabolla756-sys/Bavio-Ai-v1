...
class VoiceWorkerSession extends VoiceWorkerSessionBase {
  ...
  _finalizeTurn(status = 'success') {
    ...
    // Persist to DB asynchronously in the background (non-blocking)
    db.query(
    ...
    try {
      ...
      this._stt.onPartialTranscript((text) => {
        ...
        if (isDigitPattern || isSensitiveKeyword || isLongExplanation) {
          ...
          this.metrics.caller_speech_end_estimated = Date.now() + 1500;
          turn.caller_speech_end_estimated = Date.now() + 1500;
        } else {
          this._stt.configureThresholds({
            eotThreshold: 0.7,
            eotTimeoutMs: 600
          });
          this.metrics.caller_speech_end_estimated = Date.now() + 600;
          turn.caller_speech_end_estimated = Date.now() + 600;
        }
      });
  ...
  _handleBargeIn(source) {
    if (this.state === STATES.SPEAKING || this.state === STATES.THINKING) {
      ...
      this._log(`Barge-in detected via ${source}. Stopping active AI speech & LLM generation.`);
      this._transition(STATES.INTERRUPTED);

      // Abort active LLM generation immediately
      if (this._activeLlmAbortController) {
        this._activeLlmAbortController.abort();
        this._activeLlmAbortController = null;
      }
  ...
  async _processSpeech(transcript) {
    ...
    try {
      ...
      this._activeLlmAbortController = abortController;

      // First-token timeout (2s)
      const firstTokenTimer = setTimeout(() => {
        if (!firstTokenReceived) {
          this._log('First token timeout exceeded. Aborting.', 'error');
          this.metrics.llm_error_code = 'first_token_timeout';
          this.metrics.llm_cancelled_at = Date.now();
      ...
      const totalTimer = setTimeout(() => {
        this._log('Total response timeout exceeded. Aborting.', 'error');
        this.metrics.llm_error_code = 'total_response_timeout';
        this.metrics.llm_cancelled_at = Date.now();
        abortController.abort();
      }, 15000);
