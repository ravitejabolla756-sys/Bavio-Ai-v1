...
router.post('/twilio', rateLimiter, async (req: Request, res: Response) => {
  ...
  if (!twilioService.verifySignature(req)) {
  ...
  const payload = twilioService.parseWebhook(req);
  ...
  try {
    ...
    if (payload.eventType === 'recording') {
      logger.info(`[TWILIO WEBHOOK] Recording ready event for CallSid: ${payload.callSid}, Duration: ${payload.duration}`);
    ...
    if (payload.eventType === 'ended') {
      logger.info(`[TWILIO WEBHOOK] Call ended event for CallSid: ${payload.callSid}, Status: ${payload.status}`);
      ...
      await query(
        `UPDATE calls 
         ...
         WHERE call_sid = $3`,
        [duration, resolvedStatus, payload.callSid]
      );
    ...
    if (payload.eventType === 'incoming') {
      logger.info(`[TWILIO WEBHOOK] Incoming call event for CallSid: ${payload.callSid}`);
      ...
      if (existingCall.rows.length > 0) {
        logger.info(`[TWILIO WEBHOOK] CallSid ${payload.callSid} already logged. Skipping duplicate.`);
        return res.status(200).send('OK');
      }
      ...
      if (userRes.rows.length === 0) {
        logger.warn(`[TWILIO WEBHOOK] Virtual number ${payload.to} not registered to any user`);
      ...
      if (!isWhitelisted) {
        logger.warn(`[TWILIO WEBHOOK] Caller ${payload.from} is not whitelisted for user ${user.user_id}`);
        return res.status(403).set('Content-Type', 'text/xml').send(twilioService.generateIVRBlockTwiML());
      }
      ...
      await query(
        ...
        [user.user_id, user.country_code, payload.callSid, 'twilio', payload.from, payload.to, user.currency_code]
      );
      ...
      return res.status(200).set('Content-Type', 'text/xml').send(twilioService.generateGreetingTwiML(greeting));
    }
